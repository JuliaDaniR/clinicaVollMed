...
public class TurnoDisponibleController {
    ...
    // ==========================================================
    // 6. Listar turnos bloqueados futuros por médico
    // ==========================================================
    @Operation(
            summary = "Listar turnos bloqueados por médico",
            description = "Devuelve todos los turnos bloqueados (licencia/vacaciones) futuros del médico indicado."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Turnos bloqueados obtenidos correctamente"),
            @ApiResponse(responseCode = "404", description = "Médico no encontrado")
    })
    @GetMapping("/medico/{id}/bloqueados")
    public ResponseEntity<?> listarBloqueados(@PathVariable Long id) {
        List<TurnoDisponibleDTO> lista = turnoService.listarBloqueadosFuturos(id);
        return ResponseEntity.ok(Map.of(
                "status", "success",
                "total", lista.size(),
                "data", lista
    ...
    // ==========================================================
    // 7. Desbloquear un turno individual bloqueado
    // ==========================================================
    @Operation(
            summary = "Desbloquear un turno individual",
            description = "Pasa un turno individual de estado BLOQUEADO a DISPONIBLE."
    )
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Turno desbloqueado exitosamente"),
            @ApiResponse(responseCode = "400", description = "El turno no está bloqueado"),
            @ApiResponse(responseCode = "404", description = "Turno no encontrado")
    })
    @PostMapping("/{turnoId}/desbloquear")
    public ResponseEntity<?> desbloquearTurno(@PathVariable Long turnoId) {
        try {
            TurnoAccionDTO dto = turnoService.desbloquearTurno(turnoId);
            return ResponseEntity.ok(Map.of(
                    "status", "success",
