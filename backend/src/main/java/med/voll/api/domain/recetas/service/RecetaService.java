...
public class RecetaService {
    ...
    public ApiResponseDTO crear(DatosCrearReceta dto) {
        ...
        Medico medico = medicoRepo.findByUsuario_Id(auth.getId())
                .orElseThrow(() -> new ValidacionIntegridad("El usuario autenticado no corresponds a un médico"));
    ...
    private void validarPermisosLectura(Usuario user, RecetaMedica receta) {
        ...
        boolean esMedico = receta.getMedico().getUsuarioId() != null && receta.getMedico().getUsuarioId().equals(userId);
        
        Long pacienteUsrId = receta.getPaciente().getUsuarioId();
        boolean esPaciente = pacienteUsrId != null && pacienteUsrId.equals(userId);

        if (!esPaciente && pacienteUsrId == null) {
            // Check if user is the titular of this dependent
            esPaciente = grupoFamiliarRepo.findByPacienteDependienteId(receta.getPaciente().getId())
                    .map(gf -> gf.getPacienteTitular().getUsuarioId() != null && gf.getPacienteTitular().getUsuarioId().equals(userId))
                    .orElse(false);
        }
