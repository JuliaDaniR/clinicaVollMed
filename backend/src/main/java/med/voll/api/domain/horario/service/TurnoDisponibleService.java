...
public class TurnoDisponibleService {
    ...
    public int regenerarPorConfiguracion(Long configId) {
        ...
        return agendaGenerator.regenerarDesdeHoy(config.getMedico());
    }

    /* ============================================================
       6. LISTAR Y DESBLOQUEAR TURNOS INDIVIDUALES
       ============================================================ */
    public List<TurnoDisponibleDTO> listarBloqueadosFuturos(Long medicoId) {
        Medico medico = medicoRepo.findById(medicoId)
                .orElseThrow(() -> new IllegalArgumentException("Médico no encontrado"));

        return turnoRepo.findByMedicoIdAndEstadoAndFechaGreaterThanEqualOrderByFechaAscHoraAsc(
                medicoId, EstadoTurno.BLOQUEADO, LocalDate.now()
        ).stream()
         .map(t -> new TurnoDisponibleDTO(
                 t.getId(),
                 t.getFecha(),
    ...
    @Transactional
    public TurnoAccionDTO desbloquearTurno(Long turnoId) {
        TurnoDisponible turno = obtenerTurno(turnoId);
        if (turno.getEstado() != EstadoTurno.BLOQUEADO) {
            throw new IllegalStateException("El turno no está bloqueado");
        }
        turno.setEstado(EstadoTurno.DISPONIBLE);
        turnoRepo.save(turno);
